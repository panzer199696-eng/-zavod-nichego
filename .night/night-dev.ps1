...
# Окружение: UTF-8 (кириллица). Прокси НЕ выставляем — рабочая сессия Claude
# ходит к API напрямую через VPN AmneziaWG (порт 10808 не слушается, прокси мёртв).
# Если в будущем понадобится прокси — выставить $env:HTTPS_PROXY здесь.
$env:PYTHONUTF8 = '1'
...
# Пустой stdin (EOF сразу) — иначе claude.cmd ждёт ввод 3с при запуске без пайпа.
$null | & $claude -p $prompt --dangerously-skip-permissions *>> $log
